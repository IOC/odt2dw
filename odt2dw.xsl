...
<!-- Headline formatting -->
  ...
  <xsl:template match="//table:table">
      ...
      <xsl:variable name="tableid" select="substring-before(preceding-sibling::node()[1],'. ')"/>
      <xsl:text>::table:</xsl:text>
      <xsl:value-of select="translate($tableid,'. ','')"/>
      <xsl:value-of select="$newline"/>
      <xsl:text>  :title:</xsl:text>
      <xsl:choose>
        <xsl:when test="preceding-sibling::node()[1]">
            <xsl:value-of select="substring-after(preceding-sibling::node()[1],'. ')"/>
        </xsl:when>
      </xsl:choose>
      <xsl:value-of select="$newline"/>
      <xsl:apply-templates/>
  ...
  <xsl:template match="//table:table//table:table-row/table:covered-table-cell">
    ...
    <xsl:value-of select="$newline"/>
    <xsl:text>::table:</xsl:text>
    <xsl:value-of select="substring-before(preceding-sibling::node()[1],'.')"/>
    <xsl:value-of select="$newline"/>
    <xsl:text>  :title:</xsl:text>
  ...
  <xsl:template match="//draw:frame">
    ...
    <xsl:value-of select="$newline"/>
    <xsl:choose>
      <xsl:when test="..//draw:frame[@draw:style-name = 'Figura']">
        ...
        <xsl:if test="not (@svg:width = '' or @svg:height = '')">
          <xsl:choose><!-- check cm, px, in, etc... -->
            <xsl:when test="contains( @svg:width,'cm' )">?<xsl:value-of select="floor(number(substring-before(@svg:width,'cm')) * 37.795275591)"/>
            </xsl:when>
            <xsl:otherwise/>
          </xsl:choose>
        </xsl:if>
  ...
  <!-- Cita -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_cita']">
    <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
  ...
  <!-- Crida -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_crida']">
    <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
  ...
  <!-- Exemple -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_exemple']">
<!--     <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
    ...
    <xsl:choose>
      <xsl:when test="not(preceding-sibling::text:p[1][@text:style-name='Text_5f_exemple'])">
        <xsl:value-of select="$newline"/>
        <xsl:text>::example:</xsl:text>
        <xsl:value-of select="$newline"/>
        <xsl:text>  :title:</xsl:text>
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="following-sibling::text:p[1][@text:style-name='Text_5f_exemple'] and preceding-sibling::text:p[1][@text:style-name='Text_5f_exemple']">
            <xsl:value-of select="current()"/>
            <xsl:value-of select="$newline"/>
          </xsl:when>
          <xsl:otherwise>
  ...
  <!-- Important -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_important']">
    <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
  ...
  <!-- Complementari -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_complementari']">
<!--     <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
    ...
    <xsl:choose>
      <xsl:when test="not(preceding-sibling::text:p[1][@text:style-name='Text_5f_complementari'])">
        <xsl:value-of select="$newline"/>
        <xsl:text>::text:</xsl:text>
        <xsl:value-of select="$newline"/>
        <xsl:text>  :title:</xsl:text>
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="following-sibling::text:p[1][@text:style-name='Text_5f_complementari'] and preceding-sibling::text:p[1][@text:style-name='Text_5f_complementari']">
            <xsl:value-of select="current()"/>
            <xsl:value-of select="$newline"/>
          </xsl:when>
          <xsl:otherwise>
  ...
  <!-- Text llarg -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_llarg']">
    <!-- <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
      <xsl:text>::text:</xsl:text>
      <xsl:value-of select="$newline"/>
    ...
    <xsl:choose>
      <xsl:when test="not(preceding-sibling::text:p[1][@text:style-name='Text_5f_llarg'])">
        <xsl:value-of select="$newline"/>
        <xsl:text>::text:</xsl:text>
        <xsl:value-of select="$newline"/>
        <xsl:text>  :title:</xsl:text>
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="following-sibling::text:p[1][@text:style-name='Text_5f_llarg'] and preceding-sibling::text:p[1][@text:style-name='Text_5f_llarg']">
            <xsl:value-of select="current()"/>
            <xsl:value-of select="$newline"/>
          </xsl:when>
          <xsl:otherwise>
  ...
  <!-- Note -->
  <xsl:template match="//text:p[@text:style-name='Text_5f_nota']">
    <xsl:if test="current() != ''">
      <xsl:value-of select="$newline"/>
...
<!--   <xsl:template match="//text:span[@text:style-name='Codi_5f_linia']">
  ...
  <xsl:template match="//text:p[@text:style-name='Codi_5f_bloc']">
    <xsl:choose>
      <xsl:when test="not(preceding-sibling::text:p[1][@text:style-name='Codi_5f_bloc'])">
        <xsl:value-of select="$newline"/>
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="following-sibling::text:p[1][@text:style-name='Codi_5f_bloc'] and preceding-sibling::text:p[1][@text:style-name='Codi_5f_bloc']">
            <xsl:value-of select="current()"/>
            <xsl:value-of select="$newline"/>
          </xsl:when>
          <xsl:otherwise>
